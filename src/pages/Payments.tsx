...
export function Payments() {
    ...
    return (
        <div className="space-y-6">
            {/* Header */}
            <div>
                <p className="text-gray-600">متابعة وتسجيل دفعات الإيجار</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div className="bg-white rounded-xl shadow p-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-sm text-gray-500">مدفوعة</p>
                            <p className="text-lg font-bold text-brand-dark">{formatSAR(stats.paidAmount)}</p>
                            <p className="text-xs text-gray-400">{stats.paidCount} دفعة</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-xl shadow p-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-sm text-gray-500">مستحقة</p>
                            <p className="text-lg font-bold text-brand-dark">{formatSAR(stats.dueAmount)}</p>
                            <p className="text-xs text-gray-400">{stats.dueCount} دفعة</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-xl shadow p-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-lg font-bold text-red-600">{formatSAR(stats.overdueAmount)}</p>
                            <p className="text-xs text-gray-400">{stats.overdueCount} دفعة</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full pr-10 pl-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-blue/20 focus:border-brand-blue"
                        />
                    </div>
                    ...
                    <div className="flex gap-2">
                        {(['all', 'overdue', 'due', 'paid'] as const).map((status) => (
                            <button
                                ...
                                className={`flex-1 py-2 px-3 rounded-xl text-sm font-medium transition-all ${statusFilter === status
                    ...
                    {selectedPayment && (
                        <div className="space-y-4 mt-4">
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                    {[
                                        ...
                                        <button
                                            ...
                                            className={`py-2 px-3 rounded-lg text-sm font-medium transition-all ${paymentMethod === method.value
                                                ? 'bg-brand-blue text-white'
                                                : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                                                }`}
                                        >
                            ...
                            <div className="flex gap-3 pt-4">
                                <Button onClick={handleRecordPayment} variant="gradient" className="flex-1">
